...
export function InvoicePage({ token, onBack }: InvoicePageProps) {
  // Mock finding order by token
  const order = mockOrders.find((o) => o.token === token) || mockOrders[0];
  return (
      ...
      <div className="w-full max-w-3xl bg-white shadow-lg rounded-none md:rounded-xl overflow-hidden print:shadow-none print:w-full">
          ...
          <div className="flex justify-between items-start">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <Cake
                  size={24}
                  className="text-zm-mintCream print:text-black" />

                <h1 className="font-heading text-3xl">ZMade Cakes</h1>
              </div>
              <p className="text-zm-mintCream/80 text-sm print:text-gray-600">
                Kuwait City, Kuwait
              </p>
              <p className="text-zm-mintCream/80 text-sm print:text-gray-600">
                +965 9999 8888
        ...
        <div className="p-8 grid grid-cols-2 gap-8">
          <div>
            ...
            <p className="text-gray-600">{order.customerArea}</p>
          ...
          <div className="text-right">
            ...
            <p className="font-medium text-gray-800">{order.deliveryDate}</p>
            <p className="text-gray-600">{order.deliveryTime}</p>
            <div
              className={`inline-block mt-2 px-3 py-1 rounded-full text-xs font-bold ${order.isPaid ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b-2 border-gray-100">
                <th className="text-left py-3 text-sm font-bold text-gray-400 uppercase">
                  Item Description
                </th>
                <th className="text-center py-3 text-sm font-bold text-gray-400 uppercase w-20">
                  Qty
                </th>
                <th className="text-right py-3 text-sm font-bold text-gray-400 uppercase w-32">
                  Price
                </th>
                <th className="text-right py-3 text-sm font-bold text-gray-400 uppercase w-32">
                  Total
        ...
        <div className="p-8 text-center text-sm text-gray-400 border-t border-gray-100">
          <p>
            Thank you for choosing ZMade Cakes! We hope to sweeten your day
            again soon.
          </p>
        </div>
      </div>
