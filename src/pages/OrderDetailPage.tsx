...
export function OrderDetailPage({ orderId, onNavigate }: OrderDetailPageProps) {
  ...
  const handleShare = async () => {
    const shareData = {
      ...
      url: window.location.origin + `/view/${order.token}`
    };
    try {
      if (navigator.share) {
  ...
  const confirmAction = () => {
    if (actionType === 'status') {
      ...
      showToast(
        `Payment status updated to ${pendingValue ? 'Paid' : 'Unpaid'}`,
        'success'
      );
    }
    ...
    <div className="space-y-6 pb-20 md:pb-0">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          <Button
            variant="outline"
            size="sm"
            onClick={() => onNavigate('invoice', order.token)}>
      ...
      <div className="bg-white p-4 rounded-xl border border-zm-greyOlive/10 shadow-sm flex flex-wrap gap-4 items-center justify-between">
        <div className="flex items-center gap-4">
          <span className="text-sm font-medium text-zm-greyOlive">
            Quick Actions:
          </span>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={`
                ...
                ${order.isPaid ? 'bg-green-100 text-green-700 hover:bg-green-200' : 'bg-amber-100 text-amber-700 hover:bg-amber-200'}
              `}>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="pt-4 border-t border-zm-greyOlive/10">
                <div className="flex items-start text-zm-stoneBrown/80">
                  <MapPin
                    size={16}
                    className="mr-2 mt-1 text-zm-greyOlive shrink-0" />

                  <div>
                    <p className="font-medium">{order.customerArea}</p>
                ...
                {order.items.map((item) =>
                  <div key={item.id} className="py-4 first:pt-0 last:pb-0">
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="font-semibold text-zm-stoneBrown">
                          {item.type}
                        </h4>
                        <p className="text-sm text-zm-greyOlive">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-zm-greyOlive">
                          Qty: {item.quantity}
                        </p>
                      </div>
                    </div>
                  ...
                  {order.collateral > 0 &&
                    <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 text-sm text-blue-800 max-w-md">
                      <div className="flex items-center gap-2 font-semibold mb-1">
                        ...
                        {order.collateral.toFixed(3)} KWD
                      </div>
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to change the ${actionType} to ${actionType === 'payment' ? pendingValue ? 'Paid' : 'Unpaid' : pendingValue}?`}
