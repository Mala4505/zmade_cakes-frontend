          ...
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map((item) =>
              <button
                ...
                className={`text-sm font-medium transition-colors hover:text-zm-deepTeal ${currentPage === item.id ? 'text-zm-deepTeal font-semibold' : 'text-zm-greyOlive'}`}>
          ...
          <div className="flex flex-col space-y-4">
            {navItems.map((item) =>
              <button
                key={item.id}
                onClick={() => handleNav(item.id)}
                className={`flex items-center px-4 py-3 rounded-xl text-sm font-medium ${currentPage === item.id ? 'bg-zm-mintCream text-zm-deepTeal' : 'text-zm-stoneBrown hover:bg-gray-50'}`}>
